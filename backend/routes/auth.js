...
const generateToken = (user) => {
    const accessToken = jwt.sign(user.toObject(), process.env.ACCESS_TOKEN_SECRET, { expiresIn: '1h' });
    return {accessToken};
  }     
    ...
    User.findOne({ gtID: userData.gtID }, (error, user) => {
        if (error) {
            ...
            const newUser = new User({
                ...
                gtID: userData.gtID,
                createdAt: Date(),
                professorInfo: userData.accountType == 'professor' ? userData.professorInfo : undefined,
                studentInfo: userData.accountType == 'student' ? userData.studentInfo : undefined,
                adminInfo: userData.accountType == 'professor' ? userData.professorInfo : undefined,
            });
